...
const getAdapter = (url, path) => {
  ...
  for (const adapter of adapters) {
    if (adapter.hostname && adapter.hostname.test(hostname) && (!adapter.path || adapter.path && adapter.path.test(path))) return { ...defaults, ...adapter }
  }
  return defaults
}
...
module.exports.grabOneUrl = async (page, url, path) => {
  try {
    ...
    if (adapter.removeElementWithStyles) for (const { style, value } of adapter.removeElementWithStyles) {
    ...
    if (adapter.addStylesToElement) for (const { element, style, value } of adapter.addStylesToElement) {
      try { await page.evaluate(addStyleCleaner, element, style, value) } catch (error) { console.error(error) }
    }
    ...
    const roundedCornerResizer = adapter.fullPage
      ? sharp().extract({ left: 0, top: 0, width: 1280, height: screenshotHeight }).resize(632).jpeg({ quality: 80 })
      : sharp().resize(632).jpeg({ quality: 80 })
