...
module.exports.grabOneUrl = async (page, url, path) => {
  try {
    ...
    if (adapter.removeElements) {
      for (const removeElement of adapter.removeElements) { try { await page.evaluate(customCleaner, removeElement) } catch (error) { console.error(error) }}
    } else {
      for (const cleaner of cleaners) { try { await page.evaluate(cleaner) } catch (error) { console.error(error) } }
    }

    if (adapter.removeStyles) for (const { style, value } of adapter.removeStyles) {
      try { await page.evaluate(styleCleaner, style, value) } catch (error) { console.error(error) }
    }

    if (adapter.addStyles) {
      for (const { element, style, value } of adapter.addStyles) {
        try {
          await page.evaluate(addStyleCleaner, element, style, value)
        } catch (error) {
          console.error(error)
        }
      }
    ...
    const buffer = await page.screenshot({ encoding: 'binary', path: `${bigPath}image.jpg`, fullPage: true, type: 'jpeg', quality: 80 })

    const roundedCornerResizer = sharp().extract({ left: 0, top: 0, width: 1280, height: screenshotHeight }).resize(632).jpeg({ quality: 80 })
